...
class _BookAppointmentScreenState extends State<BookAppointmentScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (args is Appointment && !_isEditing) {
      ...
      // خدمة البداية
        _pendingServiceName = _initialAppointment!.service;
  ...
  Future<void> _loadServices() async {
    try {
      ...
      if (_pendingServiceName != null && _services.isNotEmpty) {
        final idx = _services.indexWhere(
          (s) => _serviceName(s).toLowerCase() ==
              _pendingServiceName!.toLowerCase(),
        );
        if (idx >= 0 && mounted) {
          setState(() {
            _selectedServiceIndex = idx;
          });
          _applyServiceDoctor(_services[idx]);
        }
      }
    } catch (_) {
      if (mounted) {
  ...
  String _serviceDuration(Map<String, dynamic> service) {
    return (service['duration'] ?? '').toString();
  }

  String _servicePrice(Map<String, dynamic> service) {
    return (service['price'] ?? '').toString();
  }

  String? _serviceDoctorName(Map<String, dynamic> service) {
    final value = service['doctorName'] ?? service['doctor'];
    return value?.toString();
  }

  void _applyServiceDoctor(Map<String, dynamic> service) {
    final doctorId = service['doctorId']?.toString();
    final doctorName = _serviceDoctorName(service);
  ...
  void _continueToNextStep() async {
    ...
    try {
      ...
      final serviceDoctorName = _serviceDoctorName(selectedService);
      final doctorNameToSave = _selectedDoctorName ?? serviceDoctorName;
      ...
      final appointmentData = {
        ...
        'serviceName': _serviceName(selectedService),
        'doctor': doctorNameToSave,
        'doctorId': _selectedDoctorId,
        'date': selectedDateString,
        'time': appointmentTime,
        'duration': _serviceDuration(selectedService),
        'price': _servicePrice(selectedService),
  ...
  Widget _buildServiceList() {
    ...
    if (_services.isEmpty) {
      return const Padding(
        padding: EdgeInsets.symmetric(vertical: 12),
        child: Text(
          'No services available right now. Please contact the clinic.',
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final visibleDocs = docs.where((doc) {
          final data = doc.data();
          final pending = data['pendingProfile'] == true;
          final userId = data['userId']?.toString();
          return !pending && userId != null && userId.isNotEmpty;
        }).toList();
        ...
        return DropdownButtonFormField<String>(
          ...
          onChanged: (value) {
            setState(() {
              ...
              if (value != null) {
                final chosenDoc =
                    visibleDocs.firstWhere((d) => d.id == value);
                _selectedDoctorName =

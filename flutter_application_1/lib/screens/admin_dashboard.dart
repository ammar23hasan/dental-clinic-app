...
class _DoctorsTabState extends State<_DoctorsTab> {
  ...
  static const String _secondaryAppName = 'adminDoctorCreation';
  ...
  Future<void> _requireAdmin() async {
    ...
    if (currentUser == null) {
      throw FirebaseAuthException(
        code: 'not-logged-in',
        message: 'No authenticated user found for admin action.',
      );
    }
    ...
    if (role.toLowerCase().trim() != 'admin') {
      throw FirebaseException(
        ...
        message: 'Current user is not admin. uid=${currentUser.uid}, role=$role',
      );
    }
  }
  ...
  Future<void> _createDoctorAccount({
    ...
    try {
      ...
      if (e.code == 'email-already-in-use') {
        await _linkExistingDoctorAccount(
          name: name,
          email: email,
          specialty: specialty,
        );
  ...
  Future<void> _linkExistingDoctorAccount({
    required String name,
    required String email,
    required String specialty,
  }) async {
    ...
    if (snap.docs.isEmpty) {
      await _doctorsRef.doc(email).set({
        'name': name,
        'email': email,
        'specialty': specialty,
        'userId': null,

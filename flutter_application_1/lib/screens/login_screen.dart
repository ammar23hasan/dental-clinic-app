...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<String?> _resolveRole(User user) async {
    ...
    if (role == null || role.trim().isEmpty) {
      ...
      if (email != null && email.isNotEmpty) {
        ...
        if (byEmail.docs.isNotEmpty) {
          role = byEmail.docs.first.data()['role']?.toString();
        }
      }
    }

    // Fallback: detect doctors from doctors collection and sync users/{uid}.
    if (role == null || role.trim().isEmpty) {
    ...
    if (role == null || role.trim().isEmpty) {
      final email = user.email;
      if (email != null && email.isNotEmpty) {
        final doctorByEmail = await FirebaseFirestore.instance
            .collection('doctors')
            .where('email', isEqualTo: email)
            .limit(1)
            .get();

        if (doctorByEmail.docs.isNotEmpty) {
          final data = doctorByEmail.docs.first.data();
          role = 'Doctor';
          await userDocRef.set({
            'uid': user.uid,
            'email': user.email,
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      // 2) قراءة/استرجاع الدور من Firestore
      ...
      if (role == null || role.trim().isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Role is missing. Please contact the admin.'),
            ),
          );
        }
      ...
      print('LOGIN FIRESTORE ERROR = ${e.code} - ${e.message}');
      final message = e.message ?? 'Firestore error: ${e.code}';
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(message)),
      );
    } catch (e) {
